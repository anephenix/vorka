...
function parseRoute (url, method) {
    ...
    let splitUrl    = _.compact(url.split('/'));
...
function urlMatchesRoute (splitUrl, possibleUrl) {
    let splitPossibleUrl = _.compact(possibleUrl.split('/'));
...
function routeExists (parsedRequest) {
    let splitUrl    = _.compact(parsedRequest.pathname.split('/'));    
    let possibleUrls = dynamicRoutes[parsedRequest.method][splitUrl.length];
...
function populateParamsForRequest (req, pathname, url) {
    req.params = {};
    let splitPathname   = _.compact(pathname.split('/'));    
    let splitUrl        = _.compact(url.split('/'));
    for (let i=0; i<splitUrl.length;i++) {
